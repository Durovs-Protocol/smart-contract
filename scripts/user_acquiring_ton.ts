...
export async function run(provider: NetworkProvider) {
    ...
    let assetBuilder = beginCell()
        .storeMaybeRef(
            beginCell()
                .storeAddress(Address.parse(assets[assetIndex].master))
                .storeAddress(Address.parse('UQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAJKZ'))
                .storeInt(3n, 64)
                .storeAddress(Address.parse('0QBNAja8fRL6yYP32ZuCYGolmlgsIWbbu0GtJRsvxfRo89M9'))
                .storeStringTail('9d4bff8fa95b4b588c0658fc0b2a2b09')
                .endCell(),
        )
        .endCell()
        .asSlice();
