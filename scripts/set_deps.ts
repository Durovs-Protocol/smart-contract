...
export async function run(provider: NetworkProvider) {
    ...
    const runaCoupon = provider.open(await UsdTonMaster.fromAddress(Address.parse(await loadAddress('usdTon'))));
    const manager = provider.open(await Manager.fromAddress(Address.parse(await loadAddress('manager'))));
    const profitPool = provider.open(await UsdTonMaster.fromAddress(Address.parse(await loadAddress('usdTon'))));
    ...
    async function setDeps(contract: any, name: string) {
        ...
        await timer(`Set deps in ${name}`, runaCoupon.address, usdTonAddress(contract));
    }

    // await setDeps(usdTon, 'usdTon');

    await setDeps(manager, 'new manager');
